...
namespace WiFiInput.Server
{
    public abstract class BaseControllerType
    {
        ...
        public bool isNewPacket(int packetNumber)
        {
            ...
            if (lastPacketNumber > WiFiInputConstants.CHECK_FOR_ROLLOVER_HIGH)
            {
                //if we are within a specified range also check to see if we've rolled over
                if (packetNumber < WiFiInputConstants.CHECK_FOR_ROLLOVER_LOW)
                {
            ...
            else if (lastPacketNumber < WiFiInputConstants.CHECK_FOR_ROLLOVER_LOW)
            {
                //if were at the low end of course 9999 > 1 but because we rolled over need to make sure we don't roll back
                if (packetNumber > WiFiInputConstants.CHECK_FOR_ROLLOVER_HIGH)
                {
                    return false;
                }
            }
