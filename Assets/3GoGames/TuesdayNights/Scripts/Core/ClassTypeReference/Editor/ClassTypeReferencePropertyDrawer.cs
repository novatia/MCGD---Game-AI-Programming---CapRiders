	...
	public sealed class ClassTypeReferencePropertyDrawer : PropertyDrawer {
		...
		private static string DrawTypeSelectionControl(Rect position, GUIContent label, string classRef, ClassTypeConstraintAttribute filter) {
            ...
            if (triggerDropDown)
            {
                ...
                Type ResolvedType = ResolveType(classRef);
                if (filter != null)
                {
                    DisplayDropDown(position, filteredTypes, ResolvedType, filter.Grouping);
                }
            }
		...
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
            ...
            Attribute a = fieldInfo.GetCustomAttribute(typeof(ClassTypeConstraintAttribute));
